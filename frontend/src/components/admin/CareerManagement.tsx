...
const CareerManagement = () => {
  ...
  const handleAddCareer = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!newCareerName.trim()) return alert("El nombre de la carrera no puede estar vacío.");
    try {
      // ... lógica para añadir carrera
    } catch (err: any) {
      alert(err.message);
    }
  };
  ...
  return (
    <InfoContainer>
      ...
      <form onSubmit={handleAddCareer} className="d-flex gap-2 mb-4">{/*...*/}</form>
      <hr />

      {/* --- CAMBIO 5: Nueva sección para asignar carreras --- */}
      <h2 className="mt-4 mb-3">Asignar Carrera a Alumno</h2>
      <form onSubmit={handleAssignCareer}>
        <div className="row g-3">
          <div className="col-md-5">
            <label htmlFor="student-select" className="form-label">Alumno</label>
            <select id="student-select" className="form-select" value={selectedStudentId} onChange={e => setSelectedStudentId(e.target.value)} required>
              <option value="" disabled>Selecciona un alumno...</option>
              {students.map(student => (
                <option key={student.id} value={student.id}>
                  {student.userdetail?.first_name} {student.userdetail?.last_name} ({student.username})
                </option>
              ))}
            </select>
          </div>
          <div className="col-md-5">
            ...
            <select id="career-select" className="form-select" value={selectedCareerId} onChange={e => setSelectedCareerId(e.target.value)} required>
